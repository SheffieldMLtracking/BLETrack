...
static const struct scan_configuration user_scan_conf ={
    ...
    .addr_src = GAPM_CFG_ADDR_PUBLIC,
    /// Scan interval
    .interval = 300, //60, //400,//400->250ms, //*0.625ms = 62.5ms
    /// Scan window size
    .window = 4, //6, //40, //20->12.5ms, //*0.625ms = 12.5ms
    /// Scanning mode
...
void user_adv_report_ind (struct gapm_adv_report_ind const * param ) {
  ...
  arch_printf( "%u", (int)param->report.rssi);*/
	//for (int i=0; i<param->report.rssi-150; i=i+1) {arch_printf(" "); }
	//arch_printf("|");
	//////arch_printf("GAPM_CFG_ADDR_PUBLIC = %X\r\n", GAPM_CFG_ADDR_PUBLIC);
	int rssi = (int)param->report.rssi;
	uint64_t a = GetWord32(BLE_BDADDRU_REG);
	uint64_t b = GetWord32(BLE_BDADDRL_REG);
	//arch_printf("addr: %08X %08X\r\n",a,b);
	//BLE_BDADDRL_REG 
	/** New **/
	//arch_printf(".");
	...
	while( index < param->report.data_len) {
		...
		//arch_printf( "  - ");
		// decode field type
		if(param->report.data[index + 1 ] == 0x7) {       // Services 128 bits UUID
			if (param->report.data[index+2+15]==0x12) { 
				arch_printf("FOUND no.%d rssi=%u\r\n",count,rssi);
				count++;
			}
			//arch_printf( "128 Bits UUID: ", param->report.data[index + 1 ]);
			//arch_printf("%X = 12?",param->report.data[index+2+15]);
			//for(int i= index + 2; i < index + ad_len + 1; i++) {
			//	arch_printf("%X ", param->report.data[i]);
			//}
		}
		if(param->report.data[index + 1 ] == 0x9) {                // Device name
			//if (strncmp(param->report.data, "Nordic_LBS", 10) == 0) {
			//	arch_printf("it matches!"); 
			//}
